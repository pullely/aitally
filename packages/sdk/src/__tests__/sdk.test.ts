...
describe("Transport — success path", () => {
  it("unwraps the data envelope on GET", async () => {
    ...
    const client = new Aitally({ baseUrl: "https://api.test", fetch });
...
describe("Error decoding — typed branches", () => {
  ...
  const cases: Array<[string, number, new (...args: never[]) => AitallyError]> = [
  ...
  it("forward-compat: unknown error code → base AitallyError", async () => {
    ...
    try {
      ...
      expect(err).toBeInstanceOf(AitallyError);
      expect(err).not.toBeInstanceOf(BadRequestError);
      expect((err as AitallyError).code).toBe("future_quota_exceeded");
    }
  });
