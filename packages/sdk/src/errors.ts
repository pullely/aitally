// Typed error hierarchy for the Aitally SDK.
//
...
// codes (forward compatibility) fall back to the generic `AitallyError`
...
export interface AitallyErrorInit {
 ...
 * `quota_exceeded`) decode to this class — `instanceof AitallyError`
 * still matches.
 */
export class AitallyError extends Error {
  ...
  constructor(init: AitallyErrorInit) {
    super(init.envelope.message);
    this.name = "AitallyError";
...
export class BadRequestError extends AitallyError {
  constructor(init: AitallyErrorInit) {
    super(init);
...
export class UnauthenticatedError extends AitallyError {
  constructor(init: AitallyErrorInit) {
    super(init);
...
export class ForbiddenError extends AitallyError {
  constructor(init: AitallyErrorInit) {
    super(init);
...
export class NotFoundError extends AitallyError {
  constructor(init: AitallyErrorInit) {
    super(init);
...
export class ConflictError extends AitallyError {
  constructor(init: AitallyErrorInit) {
    super(init);
...
export class ValidationError extends AitallyError {
...
export class PreconditionFailedError extends AitallyError {
  constructor(init: AitallyErrorInit) {
    super(init);
...
export class UnsupportedError extends AitallyError {
  constructor(init: AitallyErrorInit) {
    super(init);
...
export class InternalError extends AitallyError {
  constructor(init: AitallyErrorInit) {
    super(init);
...
export interface RateLimitErrorInit extends AitallyErrorInit {
...
export class RateLimitError extends AitallyError {
...
/**
 * Decode an HTTP `Response` into the appropriate `AitallyError` subclass.
 *
 * Forward-compatible: unknown error codes resolve to the base `AitallyError`.
...
export async function decodeError(
  ...
  switch (envelope.code as ErrorCode) {
    ...
    default:
      ...
      return new AitallyError(init);
  }
}

function decodeRateLimit(
  init: AitallyErrorInit,
  response: Response,
): RateLimitError {
