// Base HTTP transport for the Aitally SDK.
//
// Goals:
...
// - Typed errors: every non-2xx response is decoded into a `AitallyError`
...
export interface ClientOptions {
  /**
   * Base URL of the Aitally api-edge worker, e.g.
   * `https://api.aitally.app`. Trailing slash is stripped.
   */
  baseUrl: string;
 ...
 * `Aitally` (the client class wired up in `index.ts`).
 */
export class Transport {
  ...
  constructor(options: ClientOptions) {
    ...
    if (typeof fetchImpl !== "function") {
      throw new TypeError(
        "Aitally SDK requires a fetch implementation. Pass `fetch` in options on platforms without a global.",
      );
    }
...
export function generateRequestId(): string {
  ...
  throw new Error(
    "Aitally SDK requires the Web Crypto API (globalThis.crypto). No source of randomness was found.",
  );
}
