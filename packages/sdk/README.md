...
Aitally TypeScript SDK — typed, dependency-free, runtime-agnostic client
for the Aitally control plane API.

## Install (workspace)

This package is internal to the `aitally` monorepo. Add it as a
...
```ts
import { Aitally } from "@saas/sdk";

const client = new Aitally({
  baseUrl: "https://api.aitally.app",
  auth: { kind: "bearer", token: process.env["AITALLY_TOKEN"]! },
});
...
All non-2xx responses throw a typed subclass of `AitallyError`. Branch on
...
| `AitallyError` (base)   | _any_                 | _any_          |

Unknown error codes (forward-compatible — e.g. a future `quota_exceeded`)
decode to the base `AitallyError` carrying the raw envelope. Non-JSON 5xx
...
A custom `fetch` implementation can be injected via `new Aitally({ fetch })`.
