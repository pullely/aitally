...
`aitally` — first-class TypeScript CLI for the Aitally control
...
This package is internal to the `aitally` monorepo and is not
...
```
aitally login    [--api-url=URL] [--token=BEARER]
aitally logout
aitally whoami
```
...
```
aitally org list
aitally org use <org-id>
aitally org members
aitally project list
```

Writes (Task 0101):

```
aitally org invite <email> [--role=ROLE] [--idempotency-key=KEY] [--org=ORG_ID]
aitally project create <name> [--idempotency-key=KEY]
aitally env create <project-id> <name> [--idempotency-key=KEY]
aitally api-key create <name> [--scope=SCOPE] [--idempotency-key=KEY]
aitally webhook create <url> [--event=EVENT[,EVENT2,...]] [--idempotency-key=KEY]
```

Cross-resource reads (Task 0101):

```
aitally usage summary    [--metric=METRIC] [--from=ISO] [--to=ISO]
aitally billing summary
aitally audit list       [--limit=N] [--cursor=CURSOR] [--category=CAT] [--all]
```
...
context (`aitally org use <org-id>`). Only `org invite` accepts an
...
The shipped V1 is **token-paste**: `aitally login` prompts for a
...
- `FileTokenStore` fallback: `~/.config/aitally/credentials.json`,
  mode **0600**, parent directory mode **0700**.

Active organization context lives at
`~/.config/aitally/config.json` (mode 0644, not a secret). Override
both via `AITALLY_CONFIG_DIR` (used by tests).
