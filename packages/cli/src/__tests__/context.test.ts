...
describe("resolveConfigDir", () => {
  it("respects AITALLY_CONFIG_DIR override", () => {
    const prev = process.env["AITALLY_CONFIG_DIR"];
    process.env["AITALLY_CONFIG_DIR"] = "/tmp/cli-override";
    try {
      expect(resolveConfigDir()).toBe("/tmp/cli-override");
    } finally {
      if (prev === undefined) delete process.env["AITALLY_CONFIG_DIR"];
      else process.env["AITALLY_CONFIG_DIR"] = prev;
    }
  });
});
