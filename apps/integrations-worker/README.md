...
Your backend holds one Aitally API key and **zero GitHub credentials**.
...
const aitally = new Aitally({
  baseUrl: process.env.AITALLY_API_URL!,
  token: process.env.AITALLY_API_KEY!, // service-principal key
});

// Mint a token scoped to exactly the repos + permissions you need.
// TTL ≤ 1h; mint per task and let it expire — never store it.
const { token } = await aitally.integrations.issueGithubToken(ORG_ID, {
