...
// deleted; all wire I/O now goes through the typed `Aitally` resource
...
//   - `createClient` — constructs a `Aitally` against a target, wired
...
//     `Promise<T>` + thrown `AitallyError` into the same shape the
...
import { Aitally, AitallyError, type ClientOptions } from "@saas/sdk";
import { apiEdgeWorkersDevUrl } from "./app-config";

export type ApiClient = Aitally;
...
export function createClient(target: ApiTarget, token: string | null): Aitally {
  const opts: ClientOptions = { baseUrl: target.url };
  if (token) opts.auth = { kind: "bearer", token };
  return new Aitally(opts);
}

/**
 * Adapt an SDK promise into the `ApiResult<T>` envelope. Typed
 * `AitallyError` subclasses surface as `{ ok: false }` carrying the
